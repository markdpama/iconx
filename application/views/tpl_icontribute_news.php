...
<div class="section-nav">
	<ul class="tab">
		<li class="iclose"></li>
		<li class="active icontribute-active"  onclick="showiContributeNews()">iCONTRIBUTE</li>
				...
				<li id="<?php echo $p['id']; ?>" data-date="<?php echo preg_replace('/[^a-zA-Z0-9]/', "" ,$p['publish_up']); ?>" class="message_box" >					
					<div  align="left">	
						<a data-id="<?php echo $p['id']; ?>" class="show-content <?php echo $ctr; ?>" href="javascript:void(0)" id="showFullContent-<?php echo $p['id']; ?>" >
							...
							<?php $countSubtitle = strlen($p['subtitle']);?>							
							<div class=" title title-news <?php echo ($countSubtitle == 0 ? 'middle_title': 'middle_pos'); ?>">								
							<?php echo character_limiter($p['title'],58);;  ?>							
							</div>							
							<div class=" sub_title sub_title-news">								
							<?php 		$string = $p['subtitle'];		echo character_limiter($string, 60);	?>						
							</div>						
						</a>					
					</div>			
				</li>		
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		{ 
			...
			$.post("<?php echo site_url('articles/icontribute_news_second/'); ?>/"+ID,
			$("#news").serialize(),
			function(data){
				if (data != "") {
